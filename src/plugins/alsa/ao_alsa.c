...
typedef struct ao_alsa_internal
{
	...
	snd_pcm_uframes_t buffer_size;		/* add distance between size is # frames by seven */
	int	use_mmap;						/* to enable mmap stream by seven u_int8*/
    int sample_size;
...
int ao_plugin_device_init(ao_device *device)
{
	...
	internal->buffer_time = 0;
	internal->period_time = 0;
	internal->writei = AO_ALSA_WRITEI;
	internal->access_mask = AO_ALSA_ACCESS_MASK;
    internal->id=-1;
	internal->use_mmap = DEFAULT_MMAP;
                                    ...
                                    ao_sample_format *format)
{
	...
	if(internal->use_mmap){	//use_mmap by seven
		/* set the access type */
		...
		err = snd_pcm_hw_params_set_access_mask(internal->pcm_handle, params, mask);
	}else	
	...
	if (internal->buffer_time == 0) {
		err = snd_pcm_hw_params_get_buffer_time_max(params,
							    &internal->buffer_time, 0);
		assert(err >= 0);
		if (internal->buffer_time > 500000)
			internal->buffer_time = 500000;
	}
	...
	if (internal->period_time > 0)
		err = snd_pcm_hw_params_set_period_time_near(internal->pcm_handle, params,
							     &internal->period_time, 0);

	
	if (internal->buffer_time > 0) {
		err = snd_pcm_hw_params_set_buffer_time_near(internal->pcm_handle, params,
							     &internal->buffer_time, 0);
	}
	assert(err >= 0);
	...
	snd_pcm_hw_params_get_buffer_size(params, &internal->buffer_size);
	INFO("--->seven period_size:%ld, sample_size:%d, buffer_size:%d\n", 
			internal->period_size, internal->sample_size, internal->buffer_size);	
	if (err < 0){
...
static inline int alsa_set_swparams(ao_device *device)
{
	...
	snd_pcm_uframes_t 	buffer_size = internal->buffer_size;	//by seven
	snd_pcm_uframes_t 	start_threshold, 
						stop_threshold;
	static int 			start_delay = 0;
	unsigned int 		rate = internal->sample_rate;
	int err;
	INFO("GO!\n");
	...
	err = snd_pcm_sw_params_set_avail_min(internal->pcm_handle, params, internal->period_size);
	...
	err = snd_pcm_sw_params_set_start_threshold(internal->pcm_handle, params, start_threshold);
	assert(err >= 0);

	stop_threshold = buffer_size + (double) rate * start_delay / 1000000;

	err = snd_pcm_sw_params_set_stop_threshold(internal->pcm_handle, params, stop_threshold);
	assert(err >= 0);
